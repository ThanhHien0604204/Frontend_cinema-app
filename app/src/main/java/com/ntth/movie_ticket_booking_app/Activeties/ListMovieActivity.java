...
import com.ntth.movie_ticket_booking_app.Adapters.MovieHorizontalAdapter;
...
public class ListMovieActivity extends AppCompatActivity {
    ...
    private MovieHorizontalAdapter bannerAdapter;
    private List<Movie> movieList = new ArrayList<>();
    private List<SliderItems> bannerList = new ArrayList<>();
    ...
    private void fetchBannersFromApi() {
        api.getMoiveBanner().enqueue(new Callback<List<SliderItems>>() {
            @Override
            public void onResponse(Call<List<SliderItems>> call, Response<List<SliderItems>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    bannerList.clear();
                    bannerList.addAll(response.body());
                    bannerAdapter.notifyDataSetChanged(); // Cập nhật adapter sau khi dữ liệu thay đổi
                    //circleIndicator.setViewPager(viewPagerBanner);
                    handler.postDelayed(runnable, 3000); // Bắt đầu auto slide

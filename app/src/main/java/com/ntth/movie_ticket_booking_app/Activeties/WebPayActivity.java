...
import android.net.http.SslError;              // <— đúng package
import android.os.Bundle;
import android.os.CountDownTimer;
import android.view.View;
import android.webkit.SslErrorHandler;         // <— handler vẫn ở webkit
...
public class WebPayActivity extends AppCompatActivity {
    ...
    // DEV ONLY: cho phép bỏ qua SSL error trong sandbox (đừng bật ở production!)
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        webView.setWebViewClient(new WebViewClient() {
            ...
            @Override public void onReceivedSslError(WebView view, SslErrorHandler handler, SslError error) {
                if (DEV_ALLOW_INSECURE_SSL) {
                    // CHỈ CHO DEV/SANDBOX: chấp nhận SSL lỗi (self-signed, v.v.)
                    handler.proceed();
                } else {
                    handler.cancel(); // production: hủy nếu SSL không hợp lệ
                    Toast.makeText(WebPayActivity.this, "Lỗi SSL", Toast.LENGTH_SHORT).show();
                }
            }
            @Override public boolean shouldOverrideUrlLoading(WebView view, String url) {
                if (!url.startsWith("http")) { // deep-link sang app ZaloPay
        ...
        btnOpenExternal.setOnClickListener(v -> {
            try { startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(orderUrl))); }
            catch (Exception e) { Toast.makeText(this, "Không mở được trình duyệt", Toast.LENGTH_SHORT).show(); }
        });
                ...
                @Override public void onResponse(Call<BookingResponse> call, Response<BookingResponse> resp) {
                    ...
                    if ("CONFIRMED".equalsIgnoreCase(st)) {
                        runOnUiThread(() -> {
                            ...
                            Intent i = new Intent(WebPayActivity.this, TicketReceiptActivity.class);
                            i.putExtra(TicketReceiptActivity.EXTRA_BOOKING_ID,
